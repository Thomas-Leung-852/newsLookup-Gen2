...
const DEFAULT_SETTINGS_PATH = join(__dir, "../config/app-settings.default.json");
...
function validate(dotPath, value) {
  ...
  if (meta.type === "select") {
    const opts = (meta.options || []).map(o => (typeof o === "object" ? o.value : o));
    if (!opts.includes(value))
      return { valid: false, reason: `${key} must be one of: ${opts.join(", ")}` };
  }
...
export function loadSettings() {
  try {
    // Step 1 — load app-settings.default.json (tracked in Git, canonical schema + defaults)
    let defaults = deepMerge({}, HARDCODED_DEFAULTS);
    if (existsSync(DEFAULT_SETTINGS_PATH)) {
      const raw = readFileSync(DEFAULT_SETTINGS_PATH, "utf8");
      defaults  = deepMerge(HARDCODED_DEFAULTS, JSON.parse(raw));
      console.log("⚙️  Default settings loaded from", DEFAULT_SETTINGS_PATH);
    } else {
      console.warn("⚙️  app-settings.default.json not found — using hardcoded defaults");
    }

    // Step 2 — merge user file on top (app-settings.json — not tracked in Git)
    if (existsSync(SETTINGS_PATH)) {
      const raw  = readFileSync(SETTINGS_PATH, "utf8");
      const user = JSON.parse(raw);
      // User values win; any new keys added to defaults are automatically picked up
      _settings = deepMerge(defaults, user);
      console.log("⚙️  User settings loaded from", SETTINGS_PATH);
    } else {
      // First boot — create user file from defaults
      _settings = defaults;
      flushToDisk();
      console.log("⚙️  User settings file not found — created from defaults at", SETTINGS_PATH);
    }
  } catch (err) {
...
export function resetToDefaults() {
  ...
  // Reload from app-settings.default.json if available, else fall back to HARDCODED_DEFAULTS
  let defaults = deepMerge({}, HARDCODED_DEFAULTS);
  if (existsSync(DEFAULT_SETTINGS_PATH)) {
    try {
      const raw = readFileSync(DEFAULT_SETTINGS_PATH, "utf8");
      ...
      console.warn("⚙️  Could not read app-settings.default.json during reset:", err.message);
    }
  }
  ...
  _settings = deepMerge(defaults, meta);
  flushToDisk();
